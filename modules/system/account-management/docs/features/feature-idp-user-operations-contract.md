...
> The contract surface is `IdpProviderPluginClient` (DESIGN §3.2 and §4.1). Plugin resolution goes through `ClientHub` per the Cyber Ware gateway + plugin pattern. Providers MUST NOT silently no-op on mutating operations per PRD §5.5; unsupported mutating operations surface as `idp_unsupported_operation`.
