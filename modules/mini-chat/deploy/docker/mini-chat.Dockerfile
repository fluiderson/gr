# Multi-stage build for cyberware-server with mini-chat + k8s features
# Stage 1: Builder
...
COPY apps/cyberware-example-server ./apps/cyberware-example-server
...
# Build the cyberware-server binary.
...
RUN --mount=type=cache,target=/usr/local/cargo/registry,sharing=locked \
    ...
    if [ -n "$CARGO_FEATURES" ]; then \
        cargo build $RELEASE_FLAG --bin cyberware-server --package=cyberware-server --features "$CARGO_FEATURES"; \
    else \
        cargo build $RELEASE_FLAG --bin cyberware-server --package=cyberware-server; \
    fi && \
    cp /build/target/$OUTPUT_DIR/cyberware-example-server /tmp/cyberware-example-server
...
COPY --from=builder /tmp/cyberware-example-server /app/cyberware-example-server
# Copy config
...
USER 1000
CMD ["/app/cyberware-example-server", "--config", "/app/config/mini-chat.yaml", "run"]
