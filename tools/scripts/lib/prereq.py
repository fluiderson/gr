#!/usr/bin/env python3
"""
Prerequisite checking module for Cyber Ware testing environment.
...
class PrereqHSSrvMock(Prereq):
    def __init__(self):
        super().__init__(
            name="Cyber Ware server is running with mock mode enabled",
            remediation=(
                "Start the Cyber Ware server with mock mode enabled "
                "(use -mock option)"
            )
        )
    ...
    def check(self) -> bool:
        ...
        while time.time() - start_time < max_retry_time:
            try:
                ...
                if response.status_code != 200:
                    logging.debug(
                        f"Cyber Ware server responded with status code: "
                    ...
                    try:
                        ...
                        for service in services:
                            if service.get('name') == 'mock':
                                ...
                                logging.info(
                                    "Cyber Ware server is running with "
                                    "mock mode enabled"
                                )
                                break
                        ...
                        if mock_service_found:
                            ...
                            logging.debug(
                                "Cyber Ware server is running but mock mode "
                ...
                logging.debug(
                    f"Connection error to Cyber Ware server: {e} "
                    f"(attempt {attempt + 1})"
                )
                status = PRECHECK_WARNING
            except Exception as e:
                logging.debug(
                    f"Error checking Cyber Ware server: {e} "
                    f"(attempt {attempt + 1})"
                )
                status = PRECHECK_WARNING
        ...
        if status == PRECHECK_WARNING:
            if attempt > 0:
                logging.warning(
                    f"Cannot connect to Cyber Ware server at {BASE_URL} "
            ...
            logging.error(
                f"Cyber Ware server communication failed after "
