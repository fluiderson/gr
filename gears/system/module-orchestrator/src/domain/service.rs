...
use toolkit::registry::ModuleRegistry;
use toolkit::runtime::ModuleManager;
...
mod tests {
    ...
    use toolkit::runtime::{Endpoint, InstanceState, ModuleInstance, ModuleManager};
    use uuid::Uuid;
    ...
    impl toolkit::Module for DummyCore {
        async fn init(&self, _ctx: &toolkit::context::ModuleCtx) -> anyhow::Result<()> {
            Ok(())
        }
    }
