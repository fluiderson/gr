...
- The gateway module owns a new `MetricCatalogRepo` over `toolkit-db::DBProvider<UsageCollectorError>` with SeaORM entities, a `metric_catalog` table, embedded migrations shipped in the module crate, and a unique constraint on `gts_id`. `Module::init` wires the repo via `ctx.db_required()?`, matching the `account-management` template.
