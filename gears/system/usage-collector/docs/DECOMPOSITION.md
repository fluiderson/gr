...
- **Scope**:
  ...
  - REST API contract surface (`cpt-cf-usage-collector-interface-rest-api`) registration behind the platform API gateway and SDK trait surface (`cpt-cf-usage-collector-interface-sdk-client`) registration in ClientHub for in-process consumers. Operational telemetry is pushed via OTLP from ToolKit's global meter provider; no module-local Prometheus-scrape endpoint and no module-local health endpoints are exposed (platform liveness and readiness are handled by the ToolKit host).
...
- `usage-collector` (host module crate):
  ...
  - File layout under `src/`:
    - `module.rs` — `#[toolkit::module]` entrypoint, ClientHub wiring, REST route registration.
...
2. **Plugin `init()` publishes the instance**: each `usage-collector-plugin-<backend>` crate's `#[toolkit::module]` `init(...)` calls `PluginV1::<UsageCollectorPluginSpecV1>::build_registration("<vendor>.<package>.usage_collector_plugin.v1", cfg.vendor, cfg.priority)?` to assemble the `(instance_id, payload)` pair, registers the payload through `ctx.client_hub().get::<dyn TypesRegistryClient>()?.register(vec![payload]).await?` (gated by `RegisterResult::ensure_all_ok`), and registers the trait object via `ctx.client_hub().register_scoped::<dyn UsageCollectorPluginV1>(ClientScope::gts_id(&instance_id), api)`.
