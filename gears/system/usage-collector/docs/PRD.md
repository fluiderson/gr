...
- [ ] All authentication is performed by the ToolKit gateway upstream of the collector; the module does not implement local credential validation, MFA, SSO/federation, session management, or credential issuance, does not consume any credential-resolution contract, and rejects every REST or SDK call that arrives without a platform-resolved `SecurityContext`
