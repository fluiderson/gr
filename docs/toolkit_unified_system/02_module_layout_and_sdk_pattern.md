...
2. **Module attribute**: Enforced by the `#[toolkit::module]` macro at compile time
...
## Module Registration in Gears Server
...
your_module = { package = "cf-your-module", path = "../../gears/your-module/your-module" }
```

### 2. Import module in `apps/cf-gears-example-server/src/registered_modules.rs`
...
The `/cf` prefix comes from `modules.api-gateway.config.prefix_path` and is configurable.
...
```bash
curl -s http://127.0.0.1:8087/cf/<module>/v1/resource | python3 -m json.tool
```
...
- [ ] Register module in `apps/cf-gears-example-server/Cargo.toml` and `registered_modules.rs`.
