...
let scope = ClientScope::gts_id("gts.cf.toolkit.plugins.plugin.v1~vendor.pkg.my_module.plugin.v1~acme.test._.plugin.v1");
...
ToolKit’s plugin system enables **module + plugins** patterns where:
...
#[struct_to_gts_schema(
    dir_path = "schemas",
    base = PluginV1,
    schema_id = "gts.cf.toolkit.plugins.plugin.v1~cf.y.my_module.plugin.v1~",
...
Use `choose_plugin_instance` from `toolkit::plugins` — do **not** copy selection logic into each module.
...
#[toolkit::module(name = "my-module", deps = ["types-registry"], capabilities = [rest])]
pub struct MyModule { /* ... */ }

// Each plugin depends on types-registry
#[toolkit::module(name = "vendor-a-plugin", deps = ["types-registry"])]
pub struct VendorAPlugin { /* ... */ }
```
