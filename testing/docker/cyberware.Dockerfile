# Multi-stage build for cyberware-server API backend
# Stage 1: Builder
...
# Build the cyberware-server binary in release mode
# Using --bin to build only the specific binary
# Features can be customized via CARGO_FEATURES build arg
RUN if [ -n "$CARGO_FEATURES" ]; then \
        cargo build --release --bin cyberware-server --package=cyberware-server --features "$CARGO_FEATURES"; \
    else \
        cargo build --release --bin cyberware-server --package=cyberware-server; \
    fi
...
COPY --from=builder /build/target/release/cyberware-example-server /app/cyberware-example-server
...
USER 1000
CMD ["/app/cyberware-example-server", "--config", "/app/config/e2e-local.yaml"]
