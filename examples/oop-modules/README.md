...
cargo run --bin cyberware-server --features oop-example -- --config config/oop-example-master+follower.yaml
```
...
```bash
# Run the master with OOP modules enabled
cargo run --bin cyberware-server --features oop-example -- --config config/oop-example-master.yaml
